...
if (isset($_GET['charge_id']) || $billing_data['charge_id']) {
    ...
    if (!empty($check_charge['data']['node'])) {
        if ($check_charge['data']['node']['status'] != 'ACTIVE') {
            echo "Oh! It looks like you haven't paid our Shopify app yet. So, we can't allow you to use thr app. Apologize";
            die;
        }
    } else {
...
$query = array("query" => 'mutation {
    appSubscriptionCreate(
        ...
        test: true
        returnUrl: "https://' . $shopify->get_url() . '/admin/apps/elana-103"
        ) {
            appSubscription {
                id
            }
