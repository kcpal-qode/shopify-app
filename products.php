...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['update_id']) && $_POST['action_type'] == 'update') {
        ...
        // $update = $shopify->rest_api('/admin/api/2022-04/products/' . $_POST['update_id'] . '.json', $update_data, 'PUT');
        $update = $shopify->rest_api('/admin/api/2022-04/products/' . end($getId) . '.json', $update_data, 'PUT');
...
$products = $shopify->graphql($query);
$products = json_decode($products['body'], true);

$product_edges =  $products['data']['products'];
...
<section>
    <aside>
        ...
        <p>Fill out the following form and click submit button to create a new product.</p>
    </aside>
    <article>
        <div class="card">
